...
from data_tools.Evaluation_metrics.Exg_iou import calculate_miou
import  time
...
def fitness_func(X):
    ...
    y = X[:, 1]
    z = X[:, 2]
    mask_folder = "E:/DATA/JY/ML_DATA/GY/image"
    gt_mask_folder = "E:/DATA/JY/ML_DATA/GY/mask"
    list = []
    for i in range(x.size):
        miou = calculate_miou(x[i],y[i],z[i], mask_folder, gt_mask_folder)
        list.append(1-miou)
...
def pos():
    ...
    # 初始化各个粒子的位置
    X = np.random.uniform(-2, 2, size=(size, dim))
    # 初始化各个粒子的速度
    ...
    print("最优解是：x=%.5f,y=%.5f,z=%.5f" % (gbest[0],gbest[1],gbest[2]))
